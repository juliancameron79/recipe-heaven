...
<label for="recipe_meal">Choose Meal</label>
                <select class="form-control" id="meal" name="meal">
                    {% for mel in meal %}
                  <option value="{{mel.meal}}">{{mel.meal}}</option>
                  {%endfor%}
                </select>

                <option value="" disabled selected>Choose Meal Type</option>
                {% for mea in meal %}
                    {% if mea.meal_type == recipe.meal_type%}
                    <option value="{{mea.meal_type}}" selected >{{mea.meal_type}}</option>
                    {% else %}
                     <option value="{{mea.meal_type}}">{{mea.meal_type}}</option>
                  {% endif %}
                {% endfor %}
              </select>
...
app.config["MONGO_URI"] = 'mongodb+srv://root:r00tUserjc@myfirstcluster-0fnr8.mongodb.net/recipe_heaven?retryWrites=true&w=majority'
...
def insert_recipe():
    ...
    recipe_form = {
        ...
        "cooking_time": recipe_cookingtime,
...
def edit_recipe(recipe_id):
    ...
    meal = mongo.db.recipes.find()
    ingredients = mongo.db.recipes.ingredients.find()
    user = mongo.db.user.find()
    return render_template('editrecipe.html',
                           recipe=recipe, cuisine=cuisine, user=user, ingredients=ingredients, meal=meal)


# Submit edit recipe form
@app.route('/updat_recipe/<recipe_id>', methods=['POST'])
